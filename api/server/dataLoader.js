  ...
  loadBestiaryData(DataBase) {
    for (let besticle of best) {
      ...
      if (besticle.model == "building") {
        besticle.fields.stat_bonus = besticle.fields.stat_bonus
          .replace(/[\"\'\[\]\s]/g, "")
          .split(",");
        besticle.fields.upgrade_cost = besticle.fields.upgrade_cost
          .replace(/[\"\'\[\]\s]/g, "")
          .split(",");
      } else if (besticle.model == "monsterSkill") {
        besticle.fields.multiplier_formula_raw = besticle.fields.multiplier_formula_raw
          .replace(/[\[\]\"\s]/g, "")
          .split(",");
      }
    ...
    for (let besticle of best) {
      ...
      //if table is skills link table to effects and details
        ...
        if (
          besticle.fields.skill_effect != null &&
          besticle.fields.skill_effect.length > 0
        ) {
        ...
        if (
          besticle.fields.scaling_stats != null &&
          besticle.fields.scaling_stats.length > 0
        ) {
        ...
        //   }
        // );
        DataBase.models["MonsterSkillEffect_EffectDetail"].create({
          monsterSkillEffectDetailId: besticle.pk,
          monsterSkillEffectId: besticle.fields.effect
        });
        DataBase.models["MonsterSkill_EffectDetail"].create({
          monsterSkillEffectDetailId: besticle.pk,
          monsterSkillId: besticle.fields.skill
        });
        ...
        if (
          besticle.fields.monsters != null &&
          besticle.fields.monsters.length > 0
        ) {
